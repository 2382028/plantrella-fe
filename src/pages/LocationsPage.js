...
import "../styles/global.css"; // Aktifkan jika ada styling
...
function LocationsPage() {
  ...
  const handleDelete = async (id) => {
    if (
      window.confirm(
        `Apakah Anda yakin ingin menghapus lokasi ini (ID: ${id})?`
      )
    ) {
      setError(null);
      try {
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        <h1 className="page-title">Daftar Lokasi</h1>
        <button
          className="btn btn-primary"
          onClick={handleOpenAddForm}
          disabled={isFormVisible || loading}
        >
          Tambah Lokasi
        </button>
      </div>
      ...
      {isFormVisible && (
        <div className="card">
          <LocationForm
            ...
            onSubmit={handleSaveLocation} // <-- Prop di sini bernama onSubmit
